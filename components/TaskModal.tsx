...
export const TaskModal: React.FC<TaskModalProps> = ({
  ...
  useEffect(() => {
    if (task) {
      ...
      setAssigneeAccount(defaultAssignee !== undefined ? defaultAssignee : '');
      // SYNCHRONIZATION: Always use initialMilestoneId when passed from Milestone!
      setMilestoneId(initialMilestoneId !== undefined ? initialMilestoneId : (milestones[0]?.id || ''));
      setStatus('To do');
      setPriority('Medium');
      setNotes('');
    }
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">
                {task ? 'Chỉnh Sửa & Phân Công Task' : 'Tạo / Break Task Mới (Leader)'}
              </h3>
