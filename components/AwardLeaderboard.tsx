          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
                ...
                <span className="sm:hidden">Thưởng / Phạt Tuần</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-gradient-to-br from-red-50 to-white dark:from-red-950/70 dark:to-slate-900 border-2 border-red-300 dark:border-red-700/80 rounded-2xl p-6 shadow-sm relative overflow-hidden">
              ...
              <div className="flex items-center gap-2 text-red-700 dark:text-red-400 font-bold text-xs uppercase tracking-wider mb-2">
                ...
                🔴 DANH SÁCH PHẠT - CẢNH BÁO VI PHẠM BÁO CÁO (SAU 22H CHỦ NHẬT)
              </div>

              {lateMembers.length > 0 ? (
                <div className="space-y-3">
                  <p className="text-xs text-slate-600 dark:text-slate-300">
                    Có <strong className="text-red-600 dark:text-red-400">{lateMembers.length} thành viên</strong> chưa nộp hoặc nộp báo cáo quá mốc 22:00 Chủ Nhật:
                  </p>
                  <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                    {lateMembers.map((m) => (
                      <div
                        key={m.account}
                        className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-red-200 dark:border-red-800/80 flex items-center justify-between text-xs shadow-sm gap-2"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-950/80 border border-red-300 dark:border-red-800 flex items-center justify-center font-bold text-red-700 dark:text-red-300 shrink-0">
                            {m.account.slice(0, 2)}
                          </div>
                          <div className="min-w-0">
                            <span className="font-bold text-slate-800 dark:text-slate-100 block truncate">{m.userName} ({m.account})</span>
                            <span className="text-[10px] text-red-600 dark:text-red-400 block truncate">
                              {m.isMissingReport
                                ? `Chưa nộp báo cáo (${m.submittedCount}/${m.totalTasks} task) quá hạn 22:00 CN`
                        ...
                        <span className={`px-2.5 py-1 text-[10px] font-bold rounded-full shrink-0 ${
                          m.isMissingReport
                            ? 'bg-red-100 dark:bg-red-950/80 border border-red-300 dark:border-red-700 text-red-700 dark:text-red-300'
                            : 'bg-amber-100 dark:bg-amber-950/80 border border-amber-300 dark:border-amber-700 text-amber-800 dark:text-amber-300'
                        }`}>
                          {m.isMissingReport ? 'PHẠT (Chưa nộp)' : 'PHẠT (Nộp muộn)'}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="py-8 text-center text-emerald-600 dark:text-emerald-400 text-xs font-semibold flex flex-col items-center gap-2">
                  <CheckCircle2 className="w-8 h-8" />
                  Tất cả thành viên đều nộp báo cáo đúng hạn trước 22:00 Chủ Nhật! Không có Phạt nào.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm p-4 sm:p-5 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <span className="text-[11px] text-slate-400">Xếp theo tổng giờ thực tế</span>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider border-b border-slate-200 dark:border-slate-700">
                    ...
                    <th className="py-3 px-3 text-center font-bold text-indigo-600 dark:text-indigo-400">Tổng Effort (h)</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-200">
                  {weeklyAwards.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-center font-mono text-slate-600 dark:text-slate-300">{item.totalTasks} task</td>
                      ...
                      <td className="py-3 px-4 text-center">
                        <div className="flex items-center justify-center gap-2 flex-wrap">
                          {item.isTopEffort && (
                            <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-900/80 border border-emerald-300 dark:border-emerald-700 text-emerald-700 dark:text-emerald-200 font-bold rounded-full text-[10px] flex items-center gap-1">
                              <Award className="w-3.5 h-3.5" /> THƯỞNG (Top Effort)
                            </span>
                          )}
                          {item.isMissingReport ? (
                            <span className="px-3 py-1 bg-red-100 dark:bg-red-900/80 border border-red-300 dark:border-red-700 text-red-700 dark:text-red-200 font-bold rounded-full text-[10px] flex items-center gap-1 animate-pulse" title={item.penaltyReason}>
                              <AlertTriangle className="w-3.5 h-3.5" /> PHẠT (Chưa nộp)
                            </span>
                          ) : item.isLate ? (
                            <span className="px-3 py-1 bg-amber-100 dark:bg-amber-900/80 border border-amber-300 dark:border-amber-700 text-amber-800 dark:text-amber-200 font-bold rounded-full text-[10px] flex items-center gap-1" title={item.penaltyReason}>
                              <Clock className="w-3.5 h-3.5" /> PHẠT (Nộp muộn)
                            </span>
                          ) : item.totalTasks > 0 && item.submittedCount === item.totalTasks ? (
                            <span className="px-2.5 py-0.5 bg-emerald-50 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 rounded-full text-[10px] border border-emerald-200 dark:border-emerald-800">
                              Đã nộp ({item.submittedCount}/{item.totalTasks})
                            </span>
                          ) : (
                            <span className="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-full text-[10px] border border-slate-200 dark:border-slate-700">
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900 border border-indigo-200 dark:border-indigo-800 flex items-center justify-center text-xs font-bold text-indigo-700 dark:text-indigo-300 shrink-0">
                        {item.account.slice(0, 2)}
                      </div>
                      <div className="min-w-0">
                    ...
                    <div className="text-right shrink-0">
                      <span className="text-[10px] text-slate-400 block">Tổng Effort:</span>
                  ...
                  {/* Badges & Status */}
                  <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-700/80 flex-wrap">
                    <div className="flex items-center gap-1 flex-wrap">
                      {(item.specializations || ['BA']).map((s) => (
                        <span key={s} className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-purple-50 dark:bg-purple-950 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                          {s}
                        </span>
                      ))}
                    ...
                    <div>
                      {item.isTopEffort ? (
                        <span className="px-2.5 py-0.5 bg-emerald-100 dark:bg-emerald-900/80 border border-emerald-300 text-emerald-700 dark:text-emerald-200 font-bold rounded-full text-[10px] flex items-center gap-1">
                          <Award className="w-3 h-3" /> THƯỞNG
                        </span>
                      ) : item.isMissingReport ? (
                        <span className="px-2.5 py-0.5 bg-red-100 dark:bg-red-900/80 border border-red-300 text-red-700 dark:text-red-200 font-bold rounded-full text-[10px] flex items-center gap-1">
                          <AlertTriangle className="w-3 h-3" /> PHẠT (Chưa nộp)
                        </span>
                      ) : item.isLate ? (
                        <span className="px-2.5 py-0.5 bg-amber-100 dark:bg-amber-900/80 border border-amber-300 text-amber-800 dark:text-amber-200 font-bold rounded-full text-[10px] flex items-center gap-1">
                          <Clock className="w-3 h-3" /> PHẠT (Muộn)
                        </span>
                      ) : item.totalTasks > 0 && item.submittedCount === item.totalTasks ? (
                        <span className="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 rounded-full text-[10px] border border-emerald-200">
                          Đã nộp ({item.submittedCount}/{item.totalTasks})
                        </span>
                      ) : (
                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-full text-[10px] border border-slate-200">
